...
IMPLEMENT_DYNAMIC(CPRDuathlonDlg, CAniDialog)

BEGIN_MESSAGE_MAP(CPRDuathlonDlg, CAniDialog)
    ON_CBN_SELCHANGE(IDC_COURSE, OnSelChange)
END_MESSAGE_MAP()


CPRDuathlonDlg::CPRDuathlonDlg(CWnd* pParent /* = NULL*/) : CAniDialog(CPRDuathlonDlg::IDD, pParent)
{
}


CPRDuathlonDlg::~CPRDuathlonDlg()
{
}


BOOL CPRDuathlonDlg::OnInitDialog(void)
{
...
void CPRDuathlonDlg::OnSelChange(void)
{
    CUser* pUser = GetUser();
    if(NULL == pUser)
    {
    ...
    if((fOK) && (NULL != pCourse))
    {
        ...
        CDlgHelper::HandleDlgPaceOneTime(this, dL1PRSeconds, dLeg1Length, IDC_L1RACEPACE, FORMAT_RUN);
...
CString CPRDuathlonDlg::FindDateRun(CCourseDuathlon* pCourse, double dTime, int nQuantity)
{
    COleDateTime tm;
        ...
        for(int i = 0; i < rarDays.GetCount(); i++)
        {
            ...
            if(NULL != pDay)
            {
                ...
                for(int j = 0; j < rarWorkouts.GetCount(); j++)
                {
                    ...
                    if((NULL != pWorkout) && (RACETYPE_DUATHLON == pWorkout->GetType()))
                    {
...
void CPRDuathlonDlg::DoDataExchange(CDataExchange* pDX)
{
	CAniDialog::DoDataExchange(pDX); 
}


CUser* CPRDuathlonDlg::GetUser(void) const
{
    return m_pUser;
}


void CPRDuathlonDlg::SetUser(CUser* pUser)
{
    m_pUser = pUser;
}