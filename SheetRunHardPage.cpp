...
BEGIN_MESSAGE_MAP(CSheetRunHardPage, CSheetTriathLogPage)
...
CSheetRunHardPage::CSheetRunHardPage(void) : CSheetTriathLogPage(CSheetRunHardPage::IDD)
{
}


BOOL CSheetRunHardPage::OnInitDialog(void)
{
...
bool CSheetRunHardPage::IsChangeAllowed(void)
{
    return true;
}


bool CSheetRunHardPage::WriteContents(void)
{
    if(!::IsWindow(m_hWnd))
    {
...
void CSheetRunHardPage::InitDialogValues(void)
{
    CWorkout* pWorkout = GetWorkout();
    CUser* pUser = GetUser();
    CDayLog* pDay = GetParentSheet()->GetDay();
...
void CSheetRunHardPage::OnUpdateOKButton(void)
{
    bool fEnable = true;
...
void CSheetRunHardPage::OnUpdatePaces(void)
{
