...
BEGIN_MESSAGE_MAP(CSheetRunTempoPage, CSheetTriathLogPage)
...
CSheetRunTempoPage::CSheetRunTempoPage(void) : CSheetTriathLogPage(CSheetRunTempoPage::IDD)
{
}


BOOL CSheetRunTempoPage::OnInitDialog(void)
{
...
bool CSheetRunTempoPage::IsChangeAllowed(void)
{
    return true;
}


bool CSheetRunTempoPage::WriteContents(void)
{
    if(!::IsWindow(m_hWnd))
    {
...
void CSheetRunTempoPage::InitDialogValues(void)
{
    CWorkout* pWorkout = GetWorkout();
    CUser* pUser = GetUser();
    CDayLog* pDay = GetParentSheet()->GetDay();
...
void CSheetRunTempoPage::OnUpdateOKButton(void)
{
    bool fEnable = true;
...
void CSheetRunTempoPage::OnUpdatePaces(void)
{
