...
: m_bAni(TRUE), m_nFrames(DEF_FRAMES), m_nMillSec(CUtil::GetAnimationSpeed()),
  m_wOpenStyle(AS_CENTER), m_wCloseStyle(AS_CENTER), m_pParent(pParent),
  m_ptRefer(0, 0), m_wDispStyle(DS_WIRE), CDialog(nDlgID, pParent)
{
    EnableAni(CUtil::DoAnimations());
}

CAniDialog::CAniDialog(CPoint pt, UINT nDlgID, CWnd* pParent) 
: m_ptRefer(pt), m_nMillSec(CUtil::GetAnimationSpeed()), m_nFrames(DEF_FRAMES),
