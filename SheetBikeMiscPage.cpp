...
BEGIN_MESSAGE_MAP(CSheetBikeMiscPage, CSheetTriathLogPage)
...
CSheetBikeMiscPage::CSheetBikeMiscPage(void) : CSheetTriathLogPage(CSheetBikeMiscPage::IDD)
{
}


BOOL CSheetBikeMiscPage::OnInitDialog(void)
{
...
bool CSheetBikeMiscPage::IsChangeAllowed(void)
{
    return true;
}


bool CSheetBikeMiscPage::WriteContents(void)
{
    if(!::IsWindow(m_hWnd))
    {
...
void CSheetBikeMiscPage::InitDialogValues(void)
{
    CWorkout* pWorkout = GetWorkout();
    CUser* pUser = GetUser();
    CDayLog* pDay = GetParentSheet()->GetDay();
...
void CSheetBikeMiscPage::OnUpdateOKButton(void)
{
    bool fEnable = true;
...
void CSheetBikeMiscPage::OnUpdatePaces(void)
{
