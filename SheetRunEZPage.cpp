...
BEGIN_MESSAGE_MAP(CSheetRunEZPage, CSheetTriathLogPage)
...
CSheetRunEZPage::CSheetRunEZPage(void) : CSheetTriathLogPage(CSheetRunEZPage::IDD)
{
}


BOOL CSheetRunEZPage::OnInitDialog(void)
{
...
bool CSheetRunEZPage::IsChangeAllowed(void)
{
    return true;
}


bool CSheetRunEZPage::WriteContents(void)
{
    if(!::IsWindow(m_hWnd))
    {
...
void CSheetRunEZPage::InitDialogValues(void)
{
    CWorkout* pWorkout = GetWorkout();
    CUser* pUser = GetUser();
    CDayLog* pDay = GetParentSheet()->GetDay();
...
void CSheetRunEZPage::OnUpdateOKButton(void)
{
    bool fEnable = true;
...
void CSheetRunEZPage::OnUpdatePaces(void)
{
