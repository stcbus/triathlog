...
BEGIN_MESSAGE_MAP(CSheetBikeHardPage, CSheetTriathLogPage)
...
CSheetBikeHardPage::CSheetBikeHardPage(void) : CSheetTriathLogPage(CSheetBikeHardPage::IDD)
{
}


BOOL CSheetBikeHardPage::OnInitDialog(void)
{
...
bool CSheetBikeHardPage::IsChangeAllowed(void)
{
    return true;
}


bool CSheetBikeHardPage::WriteContents(void)
{
    if(!::IsWindow(m_hWnd))
    {
...
void CSheetBikeHardPage::InitDialogValues(void)
{
    CWorkout* pWorkout = GetWorkout();
    CUser* pUser = GetUser();
    CDayLog* pDay = GetParentSheet()->GetDay();
...
void CSheetBikeHardPage::OnUpdateOKButton(void)
{
    bool fEnable = true;
...
void CSheetBikeHardPage::OnUpdatePaces(void)
{
