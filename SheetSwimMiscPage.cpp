...
BEGIN_MESSAGE_MAP(CSheetSwimMiscPage, CSheetTriathLogPage)
...
CSheetSwimMiscPage::CSheetSwimMiscPage(void) : CSheetTriathLogPage(CSheetSwimMiscPage::IDD)
{
}


BOOL CSheetSwimMiscPage::OnInitDialog(void)
{
...
bool CSheetSwimMiscPage::IsChangeAllowed(void)
{
    return true;
}


bool CSheetSwimMiscPage::WriteContents(void)
{
    if(!::IsWindow(m_hWnd))
    {
...
void CSheetSwimMiscPage::InitDialogValues(void)
{
    CWorkout* pWorkout = GetWorkout();
    CUser* pUser = GetUser();
    CDayLog* pDay = GetParentSheet()->GetDay();
...
void CSheetSwimMiscPage::OnUpdateOKButton(void)
{
    bool fEnable = true;
...
void CSheetSwimMiscPage::OnUpdatePaces(void)
{
