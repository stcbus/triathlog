...
IMPLEMENT_DYNAMIC(CLocationSummaryRunDlg, CAniDialog)

BEGIN_MESSAGE_MAP(CLocationSummaryRunDlg, CAniDialog)
    ON_CBN_SELCHANGE(IDC_LOCATIONS, OnSelChange)
	ON_LBN_DBLCLK(IDC_DATES, OnDoubleClick)
END_MESSAGE_MAP()


CLocationSummaryRunDlg::CLocationSummaryRunDlg(CWnd* pParent /* = NULL*/) : CAniDialog(CLocationSummaryRunDlg::IDD, pParent)
{
}


CLocationSummaryRunDlg::~CLocationSummaryRunDlg()
{
}


BOOL CLocationSummaryRunDlg::OnInitDialog(void)
{
    ...
    CDlgHelper::FillComboBox((CComboBox*)GetDlgItem(IDC_LOCATIONS), pUser->GetRefLocationRunArray(), true, false);
...
void CLocationSummaryRunDlg::OnSelChange(void)
{
    CUser* pUser = GetUser();
    ...
    if((NULL != pUser) && (NULL != pLocBox))
    {
        ...
        if(nSel != CB_ERR)
        {
            LOCATIONRUNARRAY& rarLocs = pUser->GetRefLocationRunArray();
            CLocationRun* pLocation = rarLocs.GetAt(nSel);
            if(NULL != pLocation)
            {
...
void CLocationSummaryRunDlg::OnDoubleClick(void)
{
    CUser* pUser = GetUser();
...
void CLocationSummaryRunDlg::ComputeLocationData(CLocationRun* pLocation, int& rnWorkouts, double& rdMinutes, double& rdMiles)
{
    rnWorkouts = 0;
    rdMinutes = 0.0;
    rdMiles = 0.0;
        ...
        for(int i = 0; i < rarDays.GetCount(); i++)
        {
            ...
            if(NULL != pDay)
            {
                ...
                for(int j = 0; j < rarWorkouts.GetCount(); j++)
                {
                    ...
                    if(NULL != pWorkout)
                    {
                        if(pLocation == pWorkout->GetLocationRun())
                        {
...
void CLocationSummaryRunDlg::DoDataExchange(CDataExchange* pDX)
{
	CAniDialog::DoDataExchange(pDX); 
}


CUser* CLocationSummaryRunDlg::GetUser(void) const
{
    return m_pUser;
}


void CLocationSummaryRunDlg::SetUser(CUser* pUser)
{
    m_pUser = pUser;
}