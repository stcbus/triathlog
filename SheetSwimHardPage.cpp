...
BEGIN_MESSAGE_MAP(CSheetSwimHardPage, CSheetTriathLogPage)
...
CSheetSwimHardPage::CSheetSwimHardPage(void) : CSheetTriathLogPage(CSheetSwimHardPage::IDD)
{
}


BOOL CSheetSwimHardPage::OnInitDialog(void)
{
...
bool CSheetSwimHardPage::IsChangeAllowed(void)
{
    return true;
}


bool CSheetSwimHardPage::WriteContents(void)
{
    if(!::IsWindow(m_hWnd))
    {
...
void CSheetSwimHardPage::InitDialogValues(void)
{
    CWorkout* pWorkout = GetWorkout();
    CUser* pUser = GetUser();
    CDayLog* pDay = GetParentSheet()->GetDay();
...
void CSheetSwimHardPage::OnUpdateOKButton(void)
{
    bool fEnable = true;
...
void CSheetSwimHardPage::OnUpdatePaces(void)
{
