...
typedef bitset<256 * sizeof (TCHAR)>	REXA_CharSet;
...
struct REXA_LessSet{
    bool operator()(const TSet& rTSet1, const TSet& rTSet2)const
    {
        ...
        for(unsigned int i=0;i<rTSet1.size();i++){
...
struct REXA_NDFATransition{
    ...
    REXA_NDFATransition(const TCHAR* pcszChars=0,REXA_NDFAState* pNextState=0)
    {
		if(!pcszChars || _tcslen(pcszChars)==0 ){
			m_bIsMute=true;
			return;
		}
        const TCHAR* pucszChars= pcszChars;
...
public:
	...
	REXA_NDFAToDfa		(const REXA_TranslatingTable& rTable,TCHAR cEos);
...
//constructor,desctructor,typedefs
	REXA_Scanner(const TCHAR* pcszToParse=_T(""))   {SetSource(pcszToParse);}
private:
    void SetSource(const TCHAR* pcszSrc){
...
static TString       GetSymbolString (REXA_ESymbols eRegexpSymbol);
	...
	const TCHAR*		GetSource		(){return m_pcszToParse;}
	REXA_CharSet			GetCharSet		(){return m_charSet;}
    TCHAR            GetChar         (const TCHAR*& rpFrom,bool& rbIsEscape);
    void            AddRange        (TCHAR cFirst,
                                     TCHAR cLast, 
                                     REXA_CharSet& rCharSet);
    const TCHAR*     SkipWhiteSpace  ();
	const TCHAR*		m_pcszToParse;
	const TCHAR*		m_pTokBeg;
    const TCHAR*     m_pTokEnd;
	REXA_CharSet			m_charSet;
    TString          m_strLiteral;
	REXA_ESymbols   m_eLastSymbol;
};

class REXA_ParserImpl{
public:
	REXA_ParserImpl				(TCHAR cEos=0)
        :m_cEos(cEos),m_eAtomSet(eCharSet|eIdent|eLParen),
        m_nStateIdMax(0)
	{
    }
    ~REXA_ParserImpl();
    const REXA_NDFAState* ParseRegExp           (TString strRegExp);
    bool       ParseRegExpDefinition (TString strRegExpDefiniton,int nAnswer);
 
    TCHAR                    m_cEos;
    map<TString,REXA_NDFAState*>  m_mapNamedStates;
private:
...
static struct SymbolString{
    ...
    {   eEof,       _T("end of string")},
    {   eIllegal,   _T("illegal character") },
    {   eStar,      _T("*")                 },
    {   ePlus,      _T("+")                 },
    {   eAssign,    _T("=")                 },
    {   eOpt,       _T("?")                 },
    {   eAlternative,_T("|")                },
    {   eCharSet,   _T("[<characters>]")    },
    {   eLParen,    _T("(")                 },
    {   eRParen,    _T(")")                 }
};
...
{
    m_strErrMsg=   strMsg;
    m_nErrPos= static_cast<unsigned int> (pPos-pRegExp);
}
//-------------------------------------------------------------------------------
REXA_Exception::REXA_Exception(
...
//-------------------------------------------------------------------------------
{
    ...
    m_strErrMsg=  REXA_Scanner::GetSymbolString(eExpectedSymbol)    
		+  _T(" expected\r\n  ");
    m_nErrPos= static_cast<unsigned int> (token.first - pRegExp);
}
//-------------------------------------------------------------------------------
TString       REXA_Scanner::GetSymbolString     (REXA_ESymbols eRegexpSymbol)
//-------------------------------------------------------------------------------
{
...
//-------------------------------------------------------------------------------
const TCHAR*    REXA_Scanner::SkipWhiteSpace    ()
//-------------------------------------------------------------------------------
{
    const TCHAR* pCur= m_pTokEnd;
    while(isspace(*pCur))
        pCur++;
    return pCur;
}
...
//------------------------------------------------------------------------------
TCHAR          REXA_Scanner::GetChar(const TCHAR*& rpFrom,bool& rbIsEscape)
//------------------------------------------------------------------------------
{
    rbIsEscape= false;
...
//------------------------------------------------------------------------------
{
    if( (unsigned)cFirst>(unsigned)cLast ){
        swap(cFirst,cLast);
    }
    for(TCHAR c= cFirst; c<cLast;c++){
        rCharSet[c]= true;
    }
    rCharSet[(unsigned)cLast]= true;
}
//------------------------------------------------------------------------------
...
//                bool bPrevWasRangeSeparator= false;
				...
				while(	cPrev	&&	!(cPrev==']' && !bIsEscape) ){
					...
					if( !bIsEscape && c=='-'  ){
						...
						AddRange(cPrev,c,cset);
						cPrev= GetChar(m_pTokEnd,bIsEscape);
					}else{
                        cset[(unsigned)cPrev]= true;
                        cPrev= c;
					}
				}
                if( !cPrev  || !c ){
                    throw REXA_Exception(
                                m_pTokBeg,m_pcszToParse,_T("closing ] missing"));
                }
                m_charSet= cset;
				if(bUseComplement){
					m_charSet.flip();
    ...
    case '.':{
                ...
                for(int i=0;i<256 * sizeof (TCHAR);i++){
                    m_charSet[i]= true;
                }
                return m_eLastSymbol=eCharSet;
             }
    ...
    default:
        {
            if( *m_pTokEnd=='\\' && m_pTokEnd[1]=='i' ){
                ...
                bool bIsEscape;
                TCHAR c=GetChar(m_pTokEnd,bIsEscape);
                m_charSet= REXA_CharSet();
                m_charSet[(unsigned)c]= true;
                return m_eLastSymbol=eCharSet;
            }
        }
    }
...
//-----------------------------------------------------------------------------
{
    for(unsigned int i=0;i<m_vecAllStates.size();++i){
...
//-----------------------------------------------------------------------------
{
	...
	for(unsigned int i=0;i<vecAccepting.size();i++){
		vecAccepting[i]->m_nAnswer= nAnswer;
	}
    pair<map<TString,REXA_NDFAState*>::iterator,bool> itBool=
        m_mapNamedStates.insert(make_pair(TString(name.first,name.second),pState));
    if( !itBool.second ){
...
//$1 NamedRegExp= Ident "=" RegExp.
{
    ...
    if( m_scanner.GetLastSymbol()!=eEof){
         throw REXA_Exception(
                        ...
                        _T("unexpected symbol"));
    }
	InsertNamedState(name,pState,nAnswer);
}
//-----------------------------------------------------------------------------
const REXA_NDFAState*  REXA_ParserImpl::ParseRegExp          (TString strRegExp)
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
bool       REXA_ParserImpl::ParseRegExpDefinition(TString strRegExpDefiniton,
											   int nAnswer)
//-----------------------------------------------------------------------------
{
...
//$5 Atom= REXA_CharSet |  Ident | "(" RegExp ").
{
    ...
    case eCharSet:
        {
			REXA_CharSet charset = m_scanner.GetCharSet();
			REXA_NDFAState* pState= ParseCharSet(charset);
            m_scanner();
            return pState;
        }
...
//-----------------------------------------------------------------------------
{
	...
	map<TString,REXA_NDFAState*>::iterator it=
		m_mapNamedStates.find(TString(nameToFind.first,nameToFind.second));
	if( it!= m_mapNamedStates.end() ){
		return CloneNDFA(it->second);;
	}
    throw REXA_Exception(   m_scanner.m_pTokBeg,
                            m_scanner.m_pcszToParse,
                            TString(nameToFind.first,nameToFind.second-nameToFind.first)
                            + _T(" not found in name dictionary"));
}
//-----------------------------------------------------------------------------
REXA_NDFAState* REXA_ParserImpl::ParseCharSet   (REXA_CharSet& rCharSet)
//-----------------------------------------------------------------------------
{
    rCharSet[(unsigned)m_cEos]=false;
...
{ //Terms: F0 resp. F1: StateGraph reachable from 'pState0' resp. 'pState1'
	...
	for(unsigned int j=0;j<vecAccepting.size();j++){
...
//-----------------------------------------------------------------------------
{
    ...
    for(it= setStates.begin();it!=setStates.end();++it){
        for(unsigned int i=0;i<(*it)->m_transitions.size();i++){
            for(unsigned int j=0;j<(*it)->m_transitions[i].m_transChars.size();j++){
                if(     (*it)->m_transitions[i].m_transChars[j]
                    &&  isupper(j) ){
                    (*it)->m_transitions[i].
                        m_transChars[(unsigned)(tolower(j))]= true;
                }
            }
        }
    }
...
//-------------------------------------------------------------------------------
{
    ...
    if( itBool.second ){
        for(unsigned int i=0;i<(*itBool.first)->m_transitions.size();i++){
...
//-------------------------------------------------------------------------------
{
	...
	if( itDone.second ){
		...
		for(unsigned int i=0;i<pNewState->m_transitions.size();i++){
...
{
	os	<<	"\"";
	for(unsigned int i=0;i<rCharSet.size();i++){
		if( rCharSet[i] ){
			os	<<	(TCHAR)i;
		}
	}
...
//-----------------------------------------------------------------------------
{
    vector< REXA_CharSetSet > charSets(256 * sizeof (TCHAR));
    vector< REXA_CharSet >    unionSets(256 * sizeof (TCHAR));
...
//cumulative union
    for(int k= 1;k<256 * sizeof (TCHAR);k++){
        for(int j=0;j<k;j++){
...
//reduce charset into disjoint charsets
    for(int l=0;l<256 * sizeof (TCHAR);l++){
...
//-----------------------------------------------------------------------------
{
    if( rVisitedStates.find(pCurState->m_id) == rVisitedStates.end() ){
        ...
        for(unsigned int i=0;i<pCurState->m_transitions.size();i++){
...
//-----------------------------------------------------------------------------
{
    ...
    while( setPendingStateSets.size()>0 ){
        ...
        if( m_mapTransitions.find(curStateSet)==m_mapTransitions.end() ){
			...
			for(unsigned int i=0;i<m_vecCharsets.size();i++){
				REXA_StateSet::const_iterator it;
				for(it= curStateSet.begin();it!=curStateSet.end();++it){
					for(unsigned int j=0;j<(*it)->m_transitions.size();j++){	
			...
			for(unsigned int l=0;l<transVector.size();l++){
...
//-------------------------------------------------------------------------------
{
    for(int i=0;i<256 * sizeof (TCHAR);i++)	aNext[i]=0;
}
//-------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
REXA_NDFAToDfa::REXA_NDFAToDfa	(const REXA_TranslatingTable& rTable,TCHAR cEos)
						:m_rTable(rTable),m_cEos(cEos)
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
REXA_Parser::REXA_Parser     (TCHAR cEos)
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
REXA_DFAState*      REXA_Parser::ParseRegExp     (TString strRegExp)
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
void                REXA_Parser::AddRegDefinition (   TString strDefName,
                                                TString strDefExpression,
                                                int nAnswer)
//-----------------------------------------------------------------------------
{
    TString str= strDefName+ _T("=" + strDefExpression);
    m_pParserImp->ParseRegExpDefinition(str,nAnswer);
}
//-----------------------------------------------------------------------------
void                REXA_Parser::RemoveRegDefinition (   TString strDefName)
//-----------------------------------------------------------------------------
{
    m_pParserImp->m_mapNamedStates.erase(strDefName);
}
//-----------------------------------------------------------------------------
void                REXA_Parser::GetRegDefinitions   (vector<TString>& vecDefinitions)
//-----------------------------------------------------------------------------
{
    map<TString,REXA_NDFAState*>::iterator it;
