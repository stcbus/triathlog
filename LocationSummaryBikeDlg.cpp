...
IMPLEMENT_DYNAMIC(CLocationSummaryBikeDlg, CAniDialog)

BEGIN_MESSAGE_MAP(CLocationSummaryBikeDlg, CAniDialog)
    ON_CBN_SELCHANGE(IDC_LOCATIONS, OnSelChange)
	ON_LBN_DBLCLK(IDC_DATES, OnDoubleClick)
END_MESSAGE_MAP()


CLocationSummaryBikeDlg::CLocationSummaryBikeDlg(CWnd* pParent /* = NULL*/) : CAniDialog(CLocationSummaryBikeDlg::IDD, pParent)
{
}


CLocationSummaryBikeDlg::~CLocationSummaryBikeDlg()
{
}


BOOL CLocationSummaryBikeDlg::OnInitDialog(void)
{
    ...
    CDlgHelper::FillComboBox((CComboBox*)GetDlgItem(IDC_LOCATIONS), pUser->GetRefLocationBikeArray(), true, false);
...
void CLocationSummaryBikeDlg::OnSelChange(void)
{
    CUser* pUser = GetUser();
    ...
    if((NULL != pUser) && (NULL != pLocBox))
    {
        ...
        if(nSel != CB_ERR)
        {
            LOCATIONBIKEARRAY& rarLocs = pUser->GetRefLocationBikeArray();
            CLocationBike* pLocation = rarLocs.GetAt(nSel);
            if(NULL != pLocation)
            {
...
void CLocationSummaryBikeDlg::OnDoubleClick(void)
{
    CUser* pUser = GetUser();
...
void CLocationSummaryBikeDlg::ComputeLocationData(CLocationBike* pLocation, int& rnWorkouts, double& rdMinutes, double& rdMiles)
{
    rnWorkouts = 0;
    rdMinutes = 0.0;
    rdMiles = 0.0;
        ...
        for(int i = 0; i < rarDays.GetCount(); i++)
        {
            ...
            if(NULL != pDay)
            {
                ...
                for(int j = 0; j < rarWorkouts.GetCount(); j++)
                {
                    ...
                    if(NULL != pWorkout)
                    {
                        if(pLocation == pWorkout->GetLocationBike())
                        {
...
void CLocationSummaryBikeDlg::DoDataExchange(CDataExchange* pDX)
{
	CAniDialog::DoDataExchange(pDX); 
}


CUser* CLocationSummaryBikeDlg::GetUser(void) const
{
    return m_pUser;
}


void CLocationSummaryBikeDlg::SetUser(CUser* pUser)
{
    m_pUser = pUser;
}