...
BEGIN_MESSAGE_MAP(CSheetRunIntervalsPage, CSheetTriathLogPage)
...
CSheetRunIntervalsPage::CSheetRunIntervalsPage(void) : CSheetTriathLogPage(CSheetRunIntervalsPage::IDD)
{
}


BOOL CSheetRunIntervalsPage::OnInitDialog(void)
{
    ...
    if(NULL != pUser)
    {
        ...
        CDlgHelper::FillComboBox((CComboBox*)GetDlgItem(IDC_SPECIALSHOES), pUser->GetRefShoesRunArray(), true, true, true, SPORT_RUN, RUNFIELD_SPECSHOES);
...
bool CSheetRunIntervalsPage::IsChangeAllowed(void)
{
    return true;
}


bool CSheetRunIntervalsPage::WriteContents(void)
{
    if(!::IsWindow(m_hWnd))
    {
    ...
    pWorkout->SetSpecialShoesRun(CDlgHelper::GetComboSel((CComboBox*)GetDlgItem(IDC_SPECIALSHOES), pUser->GetRefShoesRunArray(), true, fOK));

    return true;
}


void CSheetRunIntervalsPage::InitDialogValues(void)
{
    CWorkout* pWorkout = GetWorkout();
    CUser* pUser = GetUser();
    CDayLog* pDay = GetParentSheet()->GetDay();
    ...
    CDlgHelper::SetComboSel((CComboBox*)GetDlgItem(IDC_SPECIALSHOES), pUser->GetRefShoesRunArray(), true, pWorkout->GetSpecialShoesRun());
...
void CSheetRunIntervalsPage::OnUpdateOKButton(void)
{
    bool fEnable = true;
...
void CSheetRunIntervalsPage::OnUpdatePaces(void)
{
