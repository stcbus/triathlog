...
BEGIN_MESSAGE_MAP(CSheetRunPoolPage, CSheetTriathLogPage)
...
CSheetRunPoolPage::CSheetRunPoolPage(void) : CSheetTriathLogPage(CSheetRunPoolPage::IDD)
{
}


BOOL CSheetRunPoolPage::OnInitDialog(void)
{
...
bool CSheetRunPoolPage::IsChangeAllowed(void)
{
    return true;
}


bool CSheetRunPoolPage::WriteContents(void)
{
    if(!::IsWindow(m_hWnd))
    {
...
void CSheetRunPoolPage::InitDialogValues(void)
{
    CWorkout* pWorkout = GetWorkout();
    CUser* pUser = GetUser();
    CDayLog* pDay = GetParentSheet()->GetDay();
...
void CSheetRunPoolPage::OnUpdateOKButton(void)
{
    bool fEnable = true;
...
void CSheetRunPoolPage::OnUpdatePaces(void)
{
