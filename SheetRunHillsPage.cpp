...
BEGIN_MESSAGE_MAP(CSheetRunHillsPage, CSheetTriathLogPage)
...
CSheetRunHillsPage::CSheetRunHillsPage(void) : CSheetTriathLogPage(CSheetRunHillsPage::IDD)
{
}


BOOL CSheetRunHillsPage::OnInitDialog(void)
{
...
bool CSheetRunHillsPage::IsChangeAllowed(void)
{
    return true;
}


bool CSheetRunHillsPage::WriteContents(void)
{
    if(!::IsWindow(m_hWnd))
    {
...
void CSheetRunHillsPage::InitDialogValues(void)
{
    CWorkout* pWorkout = GetWorkout();
    CUser* pUser = GetUser();
    CDayLog* pDay = GetParentSheet()->GetDay();
...
void CSheetRunHillsPage::OnUpdateOKButton(void)
{
    bool fEnable = true;
...
void CSheetRunHillsPage::OnUpdatePaces(void)
{
