...
IMPLEMENT_DYNAMIC(CRaceSummaryBikeDlg, CAniDialog)

BEGIN_MESSAGE_MAP(CRaceSummaryBikeDlg, CAniDialog)
    ON_CBN_SELCHANGE(IDC_COURSE, OnCourseSelChange)
    ON_CBN_SELCHANGE(IDC_DATES, OnDatesSelChange)
END_MESSAGE_MAP()


CRaceSummaryBikeDlg::CRaceSummaryBikeDlg(CWnd* pParent /* = NULL*/) : CAniDialog(CRaceSummaryBikeDlg::IDD, pParent)
{
}


CRaceSummaryBikeDlg::~CRaceSummaryBikeDlg()
{
}


BOOL CRaceSummaryBikeDlg::OnInitDialog(void)
{
...
void CRaceSummaryBikeDlg::OnCourseSelChange(void)
{
    CUser* pUser = GetUser();
    ...
    if((NULL != pUser) && (NULL != pCourseBox))
    {
        ...
        if(nSel != CB_ERR)
        {
            CCourseBike* pCourse = pUser->GetCourseBikeFromID(nSel);
            if(NULL != pCourse)
            {
...
void CRaceSummaryBikeDlg::OnDatesSelChange(void)
{
    CUser* pUser = GetUser();
    CComboBox* pCourseBox = (CComboBox*)GetDlgItem(IDC_COURSE);
    ...
    if((NULL != pUser) && (NULL != pCourseBox) && (NULL != pDateBox))
    {
        ...
        {
            CCourseBike* pCourse = pUser->GetCourseBikeFromID(nSelCourse);
            ...
            if((NULL != pCourse) && (NULL != pWorkout))
            {
                ...
                CShoesBike* pShoes = pWorkout->GetSpecialShoesBike();
                CString strShoes;
                if(NULL != pShoes)
                {
                ...
                CString strPace = CDlgHelper::FormatPace(dSeconds / 60.0, dLength, FORMAT_BIKE);
				...
				SetDlgItemText(IDC_BIKE, strBike);
...
void CRaceSummaryBikeDlg::FindDates(CCourseBike* pCourse)
{
        ...
        for(int i = 0; i < rarDays.GetCount(); i++)
        {
            ...
            if(NULL != pDay)
            {
                ...
                for(int j = 0; j < rarWorkouts.GetCount(); j++)
                {
                    ...
                    if(NULL != pWorkout)
                    {
                        if(pCourse == pWorkout->GetCourseBike())
                        {
...
void CRaceSummaryBikeDlg::DoDataExchange(CDataExchange* pDX)
{
	CAniDialog::DoDataExchange(pDX); 
}


CUser* CRaceSummaryBikeDlg::GetUser(void) const
{
    return m_pUser;
}


void CRaceSummaryBikeDlg::SetUser(CUser* pUser)
{
    m_pUser = pUser;
}