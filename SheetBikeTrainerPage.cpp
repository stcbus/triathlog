...
BEGIN_MESSAGE_MAP(CSheetBikeTrainerPage, CSheetTriathLogPage)
...
CSheetBikeTrainerPage::CSheetBikeTrainerPage(void) : CSheetTriathLogPage(CSheetBikeTrainerPage::IDD)
{
}


BOOL CSheetBikeTrainerPage::OnInitDialog(void)
{
...
bool CSheetBikeTrainerPage::IsChangeAllowed(void)
{
    return true;
}


bool CSheetBikeTrainerPage::WriteContents(void)
{
    if(!::IsWindow(m_hWnd))
    {
	...
	pWorkout->SetCountedOnBike(BST_CHECKED == IsDlgButtonChecked(IDC_COUNTONBIKE));
    pWorkout->SetCountedOnShoes(BST_CHECKED == IsDlgButtonChecked(IDC_COUNTONSHOES));
...
void CSheetBikeTrainerPage::InitDialogValues(void)
{
    CWorkout* pWorkout = GetWorkout();
    CUser* pUser = GetUser();
    CDayLog* pDay = GetParentSheet()->GetDay();
    ...
    if(pWorkout->IsCountedOnBike())
    {
        CheckDlgButton(IDC_COUNTONBIKE, BST_CHECKED);
    }

    if(pWorkout->IsCountedOnShoes())
    {
...
void CSheetBikeTrainerPage::OnUpdateOKButton(void)
{
    bool fEnable = true;
...
void CSheetBikeTrainerPage::OnUpdatePaces(void)
{
