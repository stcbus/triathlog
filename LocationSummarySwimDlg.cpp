...
IMPLEMENT_DYNAMIC(CLocationSummarySwimDlg, CAniDialog)

BEGIN_MESSAGE_MAP(CLocationSummarySwimDlg, CAniDialog)
    ON_CBN_SELCHANGE(IDC_LOCATIONS, OnSelChange)
	ON_LBN_DBLCLK(IDC_DATES, OnDoubleClick)
END_MESSAGE_MAP()


CLocationSummarySwimDlg::CLocationSummarySwimDlg(CWnd* pParent /* = NULL*/) : CAniDialog(CLocationSummarySwimDlg::IDD, pParent)
{
}


CLocationSummarySwimDlg::~CLocationSummarySwimDlg()
{
}


BOOL CLocationSummarySwimDlg::OnInitDialog(void)
{
    CAniDialog::OnInitDialog();

    SetDlgItemText(IDT_DIST, CUtil::GetSwimDistString());
    SetDlgItemText(IDT_PACE, CUtil::GetPaceString(FORMAT_SWIM, "Average Pace"));
    ...
    CDlgHelper::FillComboBox((CComboBox*)GetDlgItem(IDC_LOCATIONS), pUser->GetRefLocationSwimArray(), true, false);
...
void CLocationSummarySwimDlg::OnSelChange(void)
{
    CUser* pUser = GetUser();
    ...
    if((NULL != pUser) && (NULL != pLocBox))
    {
        ...
        if(nSel != CB_ERR)
        {
            LOCATIONSWIMARRAY& rarLocs = pUser->GetRefLocationSwimArray();
            CLocationSwim* pLocation = rarLocs.GetAt(nSel);
            if(NULL != pLocation)
            {
                ...
                ComputeLocationData(pLocation, nWorkouts, dMinutes, dMiles);
                ...
                CString strPace = CDlgHelper::FormatPace(dMinutes, dMiles, FORMAT_SWIM);
...
void CLocationSummarySwimDlg::OnDoubleClick(void)
{
    CUser* pUser = GetUser();
...
void CLocationSummarySwimDlg::ComputeLocationData(CLocationSwim* pLocation, int& rnWorkouts, double& rdMinutes, double& rdMiles)
{
    rnWorkouts = 0;
    rdMinutes = 0.0;
    rdMiles = 0.0;
        ...
        for(int i = 0; i < rarDays.GetCount(); i++)
        {
            ...
            if(NULL != pDay)
            {
                ...
                for(int j = 0; j < rarWorkouts.GetCount(); j++)
                {
                    ...
                    if(NULL != pWorkout)
                    {
                        if(pLocation == pWorkout->GetLocationSwim())
                        {
...
void CLocationSummarySwimDlg::DoDataExchange(CDataExchange* pDX)
{
	CAniDialog::DoDataExchange(pDX); 
}


CUser* CLocationSummarySwimDlg::GetUser(void) const
{
    return m_pUser;
}


void CLocationSummarySwimDlg::SetUser(CUser* pUser)
{
    m_pUser = pUser;
}