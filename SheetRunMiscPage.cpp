...
BEGIN_MESSAGE_MAP(CSheetRunMiscPage, CSheetTriathLogPage)
...
CSheetRunMiscPage::CSheetRunMiscPage(void) : CSheetTriathLogPage(CSheetRunMiscPage::IDD)
{
}


BOOL CSheetRunMiscPage::OnInitDialog(void)
{
...
bool CSheetRunMiscPage::IsChangeAllowed(void)
{
    return true;
}


bool CSheetRunMiscPage::WriteContents(void)
{
    if(!::IsWindow(m_hWnd))
    {
...
void CSheetRunMiscPage::InitDialogValues(void)
{
    CWorkout* pWorkout = GetWorkout();
    CUser* pUser = GetUser();
    CDayLog* pDay = GetParentSheet()->GetDay();
...
void CSheetRunMiscPage::OnUpdateOKButton(void)
{
    bool fEnable = true;
...
void CSheetRunMiscPage::OnUpdatePaces(void)
{
