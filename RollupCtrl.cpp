...
LRESULT CALLBACK CRollupCtrl::DlgWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (_this->m_PageHeight>r.Height())	//Can Scroll ?
	{
		...
		switch (uMsg) {
			...
			case WM_SETCURSOR:
			{
				if ((HWND)wParam==hWnd)	{ ::SetCursor(AfxGetApp()->LoadCursor(IDC_GRABHAND)); return TRUE; }
			break;
			}
...
void CRollupCtrl::OnContextMenu( CWnd* /*pWnd*/, CPoint pos )
{
	...
	if (m_cmenuCtxt.CreatePopupMenu())
	{
		...
		m_cmenuCtxt.AppendMenu(MF_STRING,		RC_MID_EXPANDALL,		"Expand All Pages"	);
		m_cmenuCtxt.AppendMenu(MF_STRING,		RC_MID_COLLAPSEALL,	"Collapse All Pages"	);
