...
class CBikeWorkoutTimeTrialDlg : public CDialog
{
public:
	CBikeWorkoutTimeTrialDlg(LOCATIONBIKEARRAY& rarLocations, SHOESBIKEARRAY& rarShoes, BIKEARRAY& rarBikes, CWnd* pParent = NULL);   
	virtual ~CBikeWorkoutTimeTrialDlg();

    virtual BOOL OnInitDialog(void);
    virtual void OnOK(void);

	enum { IDD = IDD_ADDBIKEWORKOUTTIMETRIAL };
...
protected:
    ...
    DECLARE_DYNAMIC(CBikeWorkoutTimeTrialDlg)
    DECLARE_MESSAGE_MAP()

    // data
    double m_dSeconds;
    double m_dMiles;
    double m_dTimeTrialSeconds;
    double m_dTimeTrialMiles;
    int m_nTemp;
    int m_nHR;
    int m_nWind;
    bool m_fLong;
