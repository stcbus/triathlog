...
IMPLEMENT_DYNAMIC(CRaceSummaryTriathlonDlg, CAniDialog)

BEGIN_MESSAGE_MAP(CRaceSummaryTriathlonDlg, CAniDialog)
    ON_CBN_SELCHANGE(IDC_COURSE, OnCourseSelChange)
    ON_CBN_SELCHANGE(IDC_DATES, OnDatesSelChange)
END_MESSAGE_MAP()


CRaceSummaryTriathlonDlg::CRaceSummaryTriathlonDlg(CWnd* pParent /* = NULL*/) : CAniDialog(CRaceSummaryTriathlonDlg::IDD, pParent)
{
}


CRaceSummaryTriathlonDlg::~CRaceSummaryTriathlonDlg()
{
}


BOOL CRaceSummaryTriathlonDlg::OnInitDialog(void)
{
...
void CRaceSummaryTriathlonDlg::OnCourseSelChange(void)
{
    CUser* pUser = GetUser();
    ...
    if((NULL != pUser) && (NULL != pCourseBox))
    {
        ...
        if(nSel != CB_ERR)
        {
            CCourseTriathlon* pCourse = pUser->GetCourseTriathlonFromID(nSel);
            if(NULL != pCourse)
            {
...
void CRaceSummaryTriathlonDlg::OnDatesSelChange(void)
{
    CUser* pUser = GetUser();
    CComboBox* pCourseBox = (CComboBox*)GetDlgItem(IDC_COURSE);
    ...
    if((NULL != pUser) && (NULL != pCourseBox) && (NULL != pDateBox))
    {
        ...
        {
            CCourseTriathlon* pCourse = pUser->GetCourseTriathlonFromID(nSelCourse);
            ...
            if((NULL != pCourse) && (NULL != pWorkout))
            {
                ...
                CString strPace1 = CDlgHelper::FormatPace(dSeconds1 / 60.0, dLength1, FORMAT_SWIM);
...
void CRaceSummaryTriathlonDlg::FindDates(CCourseTriathlon* pCourse)
{
...
void CRaceSummaryTriathlonDlg::DoDataExchange(CDataExchange* pDX)
{
	CAniDialog::DoDataExchange(pDX); 
}


CUser* CRaceSummaryTriathlonDlg::GetUser(void) const
{
    return m_pUser;
}


void CRaceSummaryTriathlonDlg::SetUser(CUser* pUser)
{
    m_pUser = pUser;
}