...
{
    m_nLastActive = 0;
    m_crSelected = RGB(0, 0, 200);
    m_crNormal = ::GetSysColor(COLOR_BTNTEXT);
    m_crGrayed = ::GetSysColor(COLOR_GRAYTEXT);
    m_crHilightGray = ::GetSysColor(COLOR_3DHILIGHT);
    m_crMouse = RGB(73, 146, 1);
    m_fTracking = false;
    m_nMouseTab = -1;
    m_fFontSet = false;
}
...
void CTabCtrlEx::DrawItem(LPDRAWITEMSTRUCT lpd)
{
    if(!m_fFontSet)
    {
	    ...
	    m_fntSelected.CreateFontIndirect(&lFont);
        m_fFontSet = true;
    }
    ...
    BOOL bEnabled = IsItemEnabled(nTabIndex);
    ...
    if(bEnabled)
    {
        if(m_fTracking && (nTabIndex == m_nMouseTab))
        {
            pDC->SetTextColor(m_crMouse);
        ...
        else if(bSelected)
        {
            ...
            pDC->SelectObject(&m_fntSelected);
            rect.top -= ::GetSystemMetrics(SM_CYEDGE);
            pDC->DrawText(label, rect, DT_SINGLELINE | DT_VCENTER | DT_CENTER);
        }
        else
    ...
    {
        pDC->SetTextColor(m_crHilightGray);
        pDC->DrawText(label, rect + CPoint(1, 1), DT_SINGLELINE | DT_BOTTOM | DT_CENTER);

        pDC->SetTextColor(m_crGrayed);
        pDC->DrawText(label, rect, DT_SINGLELINE | DT_BOTTOM | DT_CENTER);
    }
	...
	BOOL bIsSelected = (nTabIndex == GetCurSel());
...
//	FillRect( lpd->hDC, rect, (HBRUSH)(CTLCOLOR_DLG + 1) );
	...
	COLORREF origCol;
    int nOrigMode = SetBkMode(lpd->hDC, TRANSPARENT);
	// Draw disabled
	if ( bIsEnabled ) {

        COLORREF cr = bIsSelected ? RGB(0, 0, 200) : GetSysColor(COLOR_BTNTEXT);

		// Set the text color for our hilight (remember the original color)
        origCol = SetTextColor( lpd->hDC, cr);
...
LRESULT CTabCtrlEx::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	...
	{
        CRect rcItem;
        GetItemRect(m_nMouseTab, &rcItem);
...
{
    int nCur = GetCurSel();
    if(!IsItemEnabled(nCur))
    {
        GetParent()->PostMessage(PSM_SETCURSEL, m_nLastActive);
    }
    else
    {
        GetParent()->PostMessage(PSM_SETCURSEL, nCur);
    }
    *pResult = 0;
}
